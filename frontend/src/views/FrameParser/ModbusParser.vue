<template>
  <div class="flex flex-col bg-mk-bg w-full h-full overflow-hidden text-mk-foreground">
    <header class="flex justify-between items-center bg-mk-surface px-4 py-3 border-mk-comment border-b shrink-0">
      <h2 class="font-medium text-mk-cyan text-lg">Modbus规则设计</h2>
      <div class="flex gap-3">
        ...
        <button @click="clearAll"
          class="bg-mk-input hover:bg-mk-inputHover px-3 py-1.5 border border-mk-comment rounded-lg text-mk-yellow text-sm transition">清空所有分区</button>
      </div>
    </header>

    <!-- 数据帧输入区 -->
    <div class="p-4 border-mk-comment border-b shrink-0">
      <div class="mb-2 text-mk-comment text-sm">数据帧（十六进制，支持空格分隔）：</div>
      ...
      <div class="flex flex-wrap gap-1.5 bg-mk-input mt-3 p-3 border border-mk-comment rounded-xl">
      ...
      <div class="flex flex-wrap gap-x-4 gap-y-2 mt-3 text-sm">
        <span class="text-mk-comment">分区图例：</span>
    ...
    <div v-if="layoutMode === 'horizontal'" class="flex-1 p-4 overflow-x-auto overflow-y-hidden custom-scrollbar">
      <div class="flex flex-row items-stretch gap-3" style="min-width: min-content;">
        <div v-for="(field, idx) in fieldList" :key="idx"
          class="flex-shrink-0 bg-mk-surface p-3 border border-mk-comment/40 rounded-xl" style="width: 360px;">
          <!-- 卡片内容 -->
          <div>
            <div class="flex justify-between items-center mb-3">
              <span class="font-medium text-mk-cyan text-sm">分区 #{{ idx + 1 }}</span>
              <div class="flex gap-2">
                <button @click="moveUp(idx)" :disabled="idx === 0"
                  class="flex justify-center items-center bg-mk-input hover:bg-mk-inputHover disabled:opacity-40 rounded w-6 h-6 text-xs">↑</button>
                <button @click="moveDown(idx)" :disabled="idx === fieldList.length - 1"
                  class="flex justify-center items-center bg-mk-input hover:bg-mk-inputHover disabled:opacity-40 rounded w-6 h-6 text-xs">↓</button>
                <button @click="delField(idx)"
                  class="flex justify-center items-center bg-mk-pink/20 hover:bg-mk-pink/40 rounded w-6 h-6 text-mk-pink text-xs">×</button>
              </div>
            </div>
            <div class="flex items-center gap-3 mb-3">
              <label class="w-20 text-mk-comment text-sm">分区名称</label>
              <input v-model="field.name" placeholder="例：设备地址、功能码、数据、CRC校验"
                class="flex-1 bg-mk-input px-3 py-1.5 border border-mk-comment focus:border-mk-cyan rounded-lg focus:outline-none text-sm" />
              <label class="text-mk-comment text-sm">标记色</label>
              <input v-model="field.color" type="color"
                class="bg-transparent p-0 border-0 rounded w-8 h-8 cursor-pointer" />
            </div>
            <div class="flex items-center gap-3 mb-3">
              <label class="w-20 text-mk-comment text-sm">起始偏移(字节)</label>
              <input v-model.number="field.start" type="number" min="0"
                class="bg-mk-input px-3 py-1.5 border border-mk-comment focus:border-mk-cyan rounded-lg focus:outline-none w-24 text-sm" />
              <label class="text-mk-comment text-sm">截取长度(字节)</label>
              <input v-model.number="field.len" type="number" min="1"
                class="bg-mk-input px-3 py-1.5 border border-mk-comment focus:border-mk-cyan rounded-lg focus:outline-none w-24 text-sm" />
              <span class="text-mk-comment text-xs">字节从 0 开始计数</span>
            </div>
            <div class="bg-mk-bg mt-2 p-2 border border-mk-comment/30 rounded">
              <div class="flex items-center gap-3 mb-2"><label
                  class="flex items-center gap-1 text-sm cursor-pointer"><input type="checkbox"
                    v-model="field.enableParse" class="w-3.5 h-3.5" /><span class="text-mk-comment">启用数值解析</span></label>
              </div>
              <div v-if="field.enableParse" class="gap-3 grid grid-cols-2 text-sm">
                <div><label class="block mb-1 text-mk-comment">数值类型</label><select v-model="field.parseType"
                    class="custom-select">
                ...
                <div><label class="block mb-1 text-mk-comment">字节序</label><select v-model="field.endian"
                    class="custom-select">
                    <option value="big">大端 (Big-Endian)</option>
                    <option value="little">小端 (Little-Endian)</option>
                  </select></div>
              </div>
              <div v-if="field.enableParse" class="mt-2 pt-2 border-mk-comment/30 border-t text-sm">
                <div class="flex justify-between items-center"><span class="text-mk-comment">原始数据：</span><span
                    class="font-mono text-mk-yellow">{{ getRawHex(field) }}</span></div>
                <div class="flex justify-between items-center mt-1"><span class="text-mk-comment">解析值：</span><span
                    :class="getParseStatus(field).class" class="font-bold">{{ getParseStatus(field).text }}</span></div>
                <div v-if="getParseWarning(field)" class="mt-1 text-mk-pink text-xs">⚠️ {{ getParseWarning(field) }}
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="fieldList.length === 0" class="py-10 text-mk-comment text-center">暂无分区规则</div>
    </div>
    ...
    <div v-else class="flex-1 p-4 overflow-y-auto custom-scrollbar">
      <div class="gap-3 grid">
        <div v-for="(field, idx) in fieldList" :key="idx"
          class="bg-mk-surface p-3 border border-mk-comment/40 rounded-xl">
          <!-- 卡片内容与上面完全一致 -->
          <div>
            <div class="flex justify-between items-center mb-3">
              <span class="font-medium text-mk-cyan text-sm">分区 #{{ idx + 1 }}</span>
