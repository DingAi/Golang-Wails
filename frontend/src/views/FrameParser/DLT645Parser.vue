    ...
    <div class="flex flex-col bg-mk-bg w-full h-full overflow-hidden text-mk-foreground">
        ...
        <div class="flex-1 p-4 overflow-y-auto custom-scrollbar">
            ...
            <div v-if="parsed.link" class="bg-mk-surface mb-4 p-4 border border-mk-comment/40 rounded-xl">
                ...
                <div class="gap-3 grid grid-cols-2 md:grid-cols-3 text-sm">
                    ...
                    <div><span class="text-mk-comment">帧头校验(HCS)：</span>{{ parsed.link.hcs }} <span
                            ...
                            :class="parsed.link.hcsValid ? 'text-mk-green' : 'text-mk-pink'">({{ parsed.link.hcsValid
                            ? '通过' : '失败' }})</span></div>
                    <div><span class="text-mk-comment">整帧校验(FCS)：</span>{{ parsed.link.fcs }} <span
                            v-if="parsed.link.fcsValid !== undefined"
                            :class="parsed.link.fcsValid ? 'text-mk-green' : 'text-mk-pink'">({{ parsed.link.fcsValid
                            ? '通过' : '失败' }})</span></div>
            ...
            <div v-if="parsed.apdu" class="bg-mk-surface mb-4 p-4 border border-mk-comment/40 rounded-xl">
                ...
                <div class="text-sm">
                    <!-- APDU 类型 -->
                    <div class="mb-2"><span class="text-mk-comment">APDU类型：</span>{{ parsed.apdu.typeName }} ({{ parsed.apdu.typeId }})</div>
                    ...
                    <div v-if="parsed.apdu.oads && parsed.apdu.oads.length" class="mt-3">
                        ...
                        <div class="space-y-2 ml-2">
                            <div v-for="(oad, idx) in parsed.apdu.oads" :key="idx"
                                ...
                                <div><span class="text-mk-comment">接口类(IC)：</span>{{ oad.ic }} ({{ oad.icName }})</div>
                                <div><span class="text-mk-comment">对象标识(OI)：</span>{{ oad.oi }}</div>
                                <div><span class="text-mk-comment">属性标识(PI)：</span>{{ oad.pi }} ({{ oad.piName }})</div>
            ...
            <!-- 错误提示 -->
            <div v-if="errorMsg" class="bg-mk-pink/20 p-3 border border-mk-pink rounded-xl text-mk-pink text-sm">
                ⚠️ 解析错误：{{ errorMsg }}
            </div>

            <!-- 提示信息：推荐使用后端解析 -->
            <div v-if="!errorMsg && !parsed.link && !parsed.apdu && rawHex" class="py-10 text-mk-comment text-center">
                报文格式不正确或暂不支持自动解析。<br>
                建议通过后端 Go 服务进行完整解析。
            </div>
